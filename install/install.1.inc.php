 ...
 * $Id: install.1.inc.php 2885 2013-08-03 22:07:36Z gavinspearhead@gmail.com $
 */
...
if (!$rv_php_rec) $OUT .= ShowHelp("Recommended PHP version is " . RECOMMENDED_PHP_VERSION . " or newer");

$OUT .= '<tr><td class="install2">PHP allows "exec"</td>';
$OUT .= GenRetVal(CheckPHPExec(), $rv_php2);
if (!$rv_php2) $OUT .= ShowHelp("Please edit the file " . htmlentities($php_ini_path_cli) . " to allow use of the exec command.");
...
if (!$rv_php3) $OUT .= ShowHelp("Safe Mode is really outdated. You can disable it in the file " . htmlentities($php_ini_path_apache) . ".");

$OUT .= '<tr><td class="install2">PHP has Register Globals disabled</td>';
$OUT .= GenRetVal(CheckPHPRegGlobals(), $rv_php4);
if (!$rv_php4) $OUT .= ShowHelp("Register Globals is the biggest security hole ever. It also breaks URD in places, so you'll have to disable it in the file " .htmlentities( $php_ini_path_apache) . ".");
...
if (!$tz_cli) $OUT .= ShowHelp("unset date.timezone settings may lead to warning and date formatting problems. Please edit " .htmlentities($php_ini_path_cli));
if (!$tz_web) $OUT .= ShowHelp("unset date.timezone settings may lead to warning and date formatting problems. Please edit " .htmlentities($php_ini_path_apache));

$OUT .= '<tr><td class="install2">PHP module sockets available</td>';
$OUT .= GenRetVal(CheckPHPsockets($_SESSION['php_path']), $rv_php7);
if (!$rv_php7) $OUT .= ShowHelp("The sockets module is required by the URD daemon. Please restart the webserver after installing it");
...
if (!$rv_phpgd) $OUT .= ShowHelp("The gd module is required, needed for captchas in the registration form and statistics. Please restart the webserver after installing it");

$OUT .= '<tr><td class="install2">PHP module XMLreader/writer available</td>';
$OUT .= GenRetVal(CheckPHPXMLRW(), $rv_phpxmlrw);
if (!$rv_phpxmlrw) $OUT .= ShowHelp("The XMLreader and XMLwriter module is optional but required for backing-up URD configuration. Try: sudo pecl install XMLReader && sudo pecl install XMLwriter.  Please restart the webserver after installing it");
...
if (!$rv_php_obd) $OUT .= ShowHelp("If the open_basedir variable is set in ". htmlentities($php_ini_path_cli) . "URD might not work properly.");

$OUT .= '<tr><td class="install2">PHP Disabled functions</td>';
$OUT .= GenRetVal(check_disabled_functions($_SESSION['php_path']), $rv_php_df);
if (!$rv_php_df) $OUT .= ShowHelp("If the disabled_functions variable is set in " . htmlentities($php_ini_path_cli) . " URD might not work properly.");

$OUT .= '<tr><td class="install2">PHP Disabled classes</td>';
$OUT .= GenRetVal(check_disabled_classes($_SESSION['php_path']), $rv_php_dc);
if (!$rv_php_dc) $OUT .= ShowHelp("If the disabled_classes variable is set in $php_ini_path_cli URD might not work properly.");

$OUT .= '<tr><td class="install2">PHP memory limit &ge; 128 MB</td>';
$OUT .= GenRetVal(CheckPHPMemory($_SESSION['php_path']), $rv_php10);
if (!$rv_php10) $OUT .= ShowHelp("The current PHP memory limit (for the command line interface) is too low, please raise it to 128 MB in the (cli!) php.ini file - " . 
        htmlentities($php_ini_path_cli) . ".<br> E.g. memory_limit = 128M ; note the M here, not MB!");

if ($rv_php && $rv_php2 && $rv_php3 && $rv_php4 && $rv_php6 && $rv_php7 && $rv_php8 && $rv_php9 && $rv_php10 && $rv_phpcurl && $rv_phpgmp && $rv_phpgd) {
