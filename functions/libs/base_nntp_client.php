 ...
 * @var array
 */
private $_currentStatusResponse;
...
private function _sendCommand($cmd)
{
    ...
    // Send the command
    if ($this->_isConnected()) {
        $this->_socket->writeLine($cmd);
    } else {
        throw new exception_nntp_connect('Not connected');
    }
...
private function _getStatusResponse()
{
    ...
    $response = $this->_socket->readLine();
 ...
 */
protected function _getListResponse($nzb = NULL)
{
    $data = array();
    $line = '';
    $cnt = 0;
...
protected function _sendArticle($article)
{
 ...
 * @return mixed
 */
protected function _handleUnexpectedResponse($code = NULL, $text = NULL)
{
    if ($code === NULL) {
...
protected function connect($host, $encryption = NULL, $port = NULL, $timeout = socket::DEFAULT_SOCKET_TIMEOUT)
{
    ...
    case NNTP_PROTOCOL_RESPONSECODE_READY_POSTING_PROHIBITED: // 201, Posting NOT allowed
        ...
        break;
    case NNTP_PROTOCOL_RESPONSECODE_DISCONNECTING_FORCED:
        throw new exception("Server refused connection ({$this->_currentStatusResponse()}) ", $response);
        break;
    case NNTP_PROTOCOL_RESPONSECODE_NOT_PERMITTED: // 502, 'access restriction or permission denied' / service permanently unavailable
        throw new exception("Server refused connection ({$this->_currentStatusResponse()}) ", $response);
        break;
...
protected function cmdModeReader()
{
    ...
    case NNTP_PROTOCOL_RESPONSECODE_READY_POSTING_PROHIBITED: // 201, RFC2980: 'Hello, you can't post'
        ...
        break;
    case NNTP_PROTOCOL_RESPONSECODE_NOT_PERMITTED: // 502, 'access restriction or permission denied' / service permanently unavailable
        throw new exception("Connection being closed, since service so permanently unavailable ({$this->_currentStatusResponse()}) ", $response);
        break;
    default:
        return $this->_handleUnexpectedResponse($response);
    }
 ...
 */
protected function cmdQuit(&$msg)
{
    ...
    case 205: // RFC977: 'closing connection - goodbye!'
        ...
        if ($this->_isConnected()) {
            $this->_socket->disconnect();
            $msg = $this->_currentStatusResponse[1];
        }
        break;
    default:
...
protected function cmdGroup($newsgroup)
{
    ...
    case NNTP_PROTOCOL_RESPONSECODE_GROUP_SELECTED: // 211, RFC977: 'n f l s group selected'
        $response_arr = explode(' ', ltrim($this->_currentStatusResponse()));
    ...
    case NNTP_PROTOCOL_RESPONSECODE_NO_SUCH_GROUP: // 411, RFC977: 'no such news group'
        throw new exception("No such news group ({$this->_currentStatusResponse()}) " , $response);
        break;
    default:
        return $this->_handleUnexpectedResponse($response);
    }
...
protected function cmdListgroup($newsgroup = NULL, $range = NULL)
{
    ...
    case NNTP_PROTOCOL_RESPONSECODE_GROUP_SELECTED: // 211, RFC2980: 'list of article numbers follow'
        ...
        $response_arr = explode(' ', ltrim($this->_currentStatusResponse()), 4);
    ...
    case NNTP_PROTOCOL_RESPONSECODE_NO_GROUP_SELECTED: // 412, RFC2980: 'Not currently in newsgroup'
        throw new exception("Not currently in newsgroup ({$this->_currentStatusResponse()}) ", $response);
        break;
    case 502: // RFC2980: 'no permission'
        throw new exception("No permission ({$this->_currentStatusResponse()}) ", $response);
        break;
    default:
        return $this->_handleUnexpectedResponse($response);
    }
...
protected function cmdLast()
{
    ...
    case NNTP_PROTOCOL_RESPONSECODE_NO_GROUP_SELECTED: // 412, RFC977: 'no newsgroup selected'
        throw new exception("No newsgroup has been selected ({$this->_currentStatusResponse()}) ", $response);
        break;
    case NNTP_PROTOCOL_RESPONSECODE_NO_ARTICLE_SELECTED: // 420, RFC977: 'no current article has been selected'
        throw new exception("No current article has been selected ({$this->_currentStatusResponse()}) ", $response);
        break;
    case NNTP_PROTOCOL_RESPONSECODE_NO_PREVIOUS_ARTICLE: // 422, RFC977: 'no previous article in this group'
        throw new exception("No previous article in this group ({$this->_currentStatusResponse()}) ", $response);
        break;
    default:
        return $this->_handleUnexpectedResponse($response);
    }
...
protected function cmdNext()
{
    ...
    case NNTP_PROTOCOL_RESPONSECODE_NO_NEXT_ARTICLE: // 421, RFC977: 'no next article in this group'
        throw new exception("No next article in this group ({$this->_currentStatusResponse()}) ", $response);
        break;
    default:
        return $this->_handleUnexpectedResponse($response);
    }
...
protected function cmdArticle($article = NULL)
{
    ...
    case NNTP_PROTOCOL_RESPONSECODE_NO_SUCH_ARTICLE_NUMBER: // 423, RFC977: 'no such article number in this group'
        throw new exception("No such article number in this group ({$this->_currentStatusResponse()}) ", $response);
        break;
    case NNTP_PROTOCOL_RESPONSECODE_NO_SUCH_ARTICLE_ID: // 430, RFC977: 'no such article found'
        throw new exception("No such article found ({$this->_currentStatusResponse()}) ", $response);
        break;
    default:
        return $this->_handleUnexpectedResponse($response);
    }
...
protected function cmdBodyAdv($article, article_writer &$aw)
{
    ...
    case NNTP_PROTOCOL_RESPONSECODE_BODY_FOLLOWS:     // 222, RFC977: 'n <a> article retrieved - body follows'
        ...
        return $this->_getTextResponseAdv($aw);
        break;
    case NNTP_PROTOCOL_RESPONSECODE_NO_GROUP_SELECTED: // 412, RFC977: 'no newsgroup has been selected'
        throw new exception("No newsgroup has been selected ({$this->_currentStatusResponse()})", $response);
        break;
...
protected function cmdPost()
{
    ...
    case NNTP_PROTOCOL_RESPONSECODE_POSTING_PROHIBITED: // 440, RFC977: 'posting not allowed'
        throw new exception("Posting not allowed ({$this->_currentStatusResponse()})", $response);
        break;
    default:
        return $this->_handleUnexpectedResponse($response);
    }
 ...
 * @return mixed (bool) true on success or (object) pear_error on failure
 */
protected function cmdPost2($article, &$articleid)
{
    ...
    case NNTP_PROTOCOL_RESPONSECODE_POSTING_SUCCESS: // 240, RFC977: 'article posted ok'
        $resp = $this->_currentStatusResponse[1];
    ...
    case NNTP_PROTOCOL_RESPONSECODE_POSTING_FAILURE: // 441, RFC977: 'posting failed'
        throw new exception("Posting failed ({$this->_currentStatusResponse()})", $response);
        break;
    default:
        return $this->_handleUnexpectedResponse($response);
    }
...
protected function cmdIhave($id)
{
    ...
    case NNTP_PROTOCOL_RESPONSECODE_TRANSFER_UNWANTED: // 435
        throw new exception("Article not wanted ({$this->_currentStatusResponse()})", $response);
        break;
    case NNTP_PROTOCOL_RESPONSECODE_TRANSFER_FAILURE: // 436
        throw new exception("Transfer not possible; try again later ({$this->_currentStatusResponse()})", $response);
        break;
    default:
        return $this->_handleUnexpectedResponse($response);
    }
...
protected function cmdIhave2($article)
{
    ...
    case NNTP_PROTOCOL_RESPONSECODE_TRANSFER_REJECTED: // 437
        throw new exception("Transfer rejected; do not retry ({$this->_currentStatusResponse()})", $response);
        break;
    default:
        return $this->_handleUnexpectedResponse($response);
    }
 ...
 * @return mixed (array) nested array with informations about existing newsgroups on success or (object) pear_error on failure
 */
protected function cmdList($db = NULL)
{
...
protected function cmdListActive($db, $wildmat = NULL)
{
    $command = 'LIST ACTIVE';
    if (!is_null($wildmat))
        $command .= ' ' . $wildmat;
    ...
    case NNTP_PROTOCOL_RESPONSECODE_GROUPS_FOLLOW: // 215, RFC977: 'list of newsgroups follows'
        ...
        return $this->_getListResponse($db);
        break;
    default:
        return $this->_handleUnexpectedResponse($response);
    }
...
protected function cmdListNewsgroups($wildmat = NULL)
{
    ...
    case 503: // RFC2980: 'program error, function not performed'
        throw new exception('Internal server error, function not performed', $response, $this->_currentStatusResponse());
        break;
    default:
        return $this->_handleUnexpectedResponse($response);
    }
...
protected function cmdOver($range = NULL)
{
    ...
    case NNTP_PROTOCOL_RESPONSECODE_NO_GROUP_SELECTED: // 412, RFC2980: 'No news group current selected'
        throw new exception("No news group current selected ({$this->_currentStatusResponse()})", $response);
        break;
    case NNTP_PROTOCOL_RESPONSECODE_NO_ARTICLE_SELECTED: // 420, RFC2980: 'No article(s) selected'
        throw new exception("No article(s) selected ({$this->_currentStatusResponse()})", $response);
        break;
    case NNTP_PROTOCOL_RESPONSECODE_NO_SUCH_ARTICLE_NUMBER: // 423:, Draft27: 'No articles in that range'
        throw new exception("No articles in that range ({$this->_currentStatusResponse()})", $response);
        break;
    case 502: // RFC2980: 'no permission'
        throw new exception("No permission ({$this->_currentStatusResponse()})", $response);
        break;
...
protected function cmdXZver($range = NULL)
{
    ...
    case NNTP_PROTOCOL_RESPONSECODE_UNKNOWN_COMMAND: // 500  RFC2980: 'unknown command'
        throw new exception("XZver not supported ({$this->_currentStatusResponse()})", $response);
        break;
    default:
        return $this->_handleUnexpectedResponse($response);
    }
...
protected function cmdFastXZver($range = NULL)
{
    $command = 'XZVER';
    if (!is_null($range)) {
        $command .= ' ' . $range;
...
protected function cmdFastXOver($range = NULL)
{
    $command = 'XOVER';
    if (!is_null($range)) {
        $command .= ' ' . $range;
...
protected function cmdListOverviewFmt()
{
    ...
    case NNTP_PROTOCOL_RESPONSECODE_NOT_SUPPORTED: // RFC2980: 'program error, function not performed'
        throw new exception("Internal server error, function not performed ({$this->_currentStatusResponse()})", $response);
        break;
    default:
        return $this->_handleUnexpectedResponse($response);
    }
...
protected function cmdXHdr($field, $range = NULL)
{
    ...
    case NNTP_PROTOCOL_RESPONSECODE_NO_ARTICLE_SELECTED: // 420, RFC2980: 'No current article selected'
        throw new exception("No current article selected ({$this->_currentStatusResponse()})", $response);
        break;
    case NNTP_PROTOCOL_RESPONSECODE_NO_SUCH_ARTICLE_ID: // 430, RFC2980: 'No such article'
        throw new exception("No such article ({$this->_currentStatusResponse()})", $response);
        break;
    case NNTP_PROTOCOL_RESPONSECODE_NOT_PERMITTED: // RFC2980: 'no permission'
        throw new exception("No permission ({$this->_currentStatusResponse()})", $response);
        break;
...
protected function cmdXGTitle($wildmat = '*')
{
    ...
    case NNTP_PROTOCOL_RESPONSECODE_GROUPS_UNAVAILABLE: // RFC2980: 'Groups and descriptions unavailable'
        throw new exception("Groups and descriptions unavailable ({$this->_currentStatusResponse()})", $response);
        break;
    default:
        return $this->_handleUnexpectedResponse($response);
    }
...
protected function cmdXPat($field, $range, $wildmat)
{
    ...
    case NNTP_PROTOCOL_RESPONSECODE_HEAD_FOLLOWS: // 221, RFC2980: 'Header follows'
        ...
        break;
    case NNTP_PROTOCOL_RESPONSECODE_NO_SUCH_ARTICLE_ID: // 430, RFC2980: 'No such article'
        throw new exception("No current article selected ({$this->_currentStatusResponse()})");
        break;
    case NNTP_PROTOCOL_RESPONSECODE_NOT_PERMITTED: // RFC2980: 'no permission'
        throw new exception("No permission ({$this->_currentStatusResponse()})", $response);
        break;
...
protected function cmdAuthinfo($user, $pass)
{
    ...
    case NNTP_PROTOCOL_RESPONSECODE_AUTHENTICATION_CONTINUE: // RFC2980: 'More authentication information required'
        throw new exception("Authentication uncompleted ({$this->_currentStatusResponse()})", $response);
        break;
    case NNTP_PROTOCOL_RESPONSECODE_AUTHENTICATION_REJECTED: // RFC2980: 'Authentication rejected'
        throw new exception("Authentication rejected ({$this->_currentStatusResponse()})", $response);
        break;
    case NNTP_PROTOCOL_RESPONSECODE_NOT_PERMITTED: // RFC2980: 'No permission'
        throw new exception("Authentication rejected ({$this->_currentStatusResponse()})", $response);
        break;
    ...
    case NNTP_PROTOCOL_RESPONSECODE_SYNTAX_ERROR:
        throw new exception('Authentication failed', $response, $this->_currentStatusResponse());
        break;
    default:
        return $this->_handleUnexpectedResponse($response);
    }
