    ...
    $.post(url, {
        ...
        if (basket_type == 1) { // the normal basket
            ...
            if ($('#basketbuttondiv') !== undefined) {
                $('#minibasketdiv').hide();
                if (content === '') {
                    $('#basketbuttondiv').hide();
                } else {
                    $('#basketdiv').show();
                    $('#basketbuttondiv').show();
                }
            }
            $('#minibasketdiv').html('');
            ...
            $('#dir_select_button').click( function() { $('#dir_select_span').toggle(); $('#dl_dir_span').toggle(); });
...
function submit_viewfiles_action(fileid, command)
{
    ...
    if (command == 'up_nzb') {
        ...
        $('body').append(ifr);
        $('#' + id).on('load',function() {
                    var msg = $('#' + id).contents().find("body").html();
...
function _show_overlayed_content(html, style, content, back, close_button)
{
    ...
    if (html.substr(0, 7) == ':error:') {
        ...
        if (c_width > w_width) {
            $(content).css({ 
                'width': w_width,
                'marginLeft': (- Math.floor(w_width / 2)),
                'left': '50%' });
        }
        if (c_height > w_height) {
            $(content).css({
                'height': w_height,
                'marginTop': (- Math.floor(w_height / 2)),
                'top': '50%'});
        }
    }
}
...
function show_uploadnzb(dir, name)
{
    ...
    $.post(url, data).done(function(html) {
        ...
        if (x.error == 0) {
            ...
            $('#timestamp').keyup( function () { hide_popup('calendardiv', 'calendar'); });
            $('#toggle_button').click( function () { $('#dir_select_span').toggle(); $('#dl_dir_span').toggle(); });
        } else {
            update_message_bar(x.error);
        }
    });
...
function search_button(url, xname)
{
    ...
    if (srch == '') {
        ...
        srch = srch.replace(/[\]\[_"+.']/g, ' ');
        srch = srch.replace(/^\s+/, '');
        url = url.replace(/\$q/, encodeURIComponent(srch));
        var w = window.open()
        w.name = xname + 'window'
        w.opener = null;
        w.location = url;
...
function update_rss()
{
    ...
    if (!$('#needauthentication_img').hasClass('checkbox_on') ) {
        data.rss_password = '';
        data.rss_username = '';
    }
    $.post(url, data).done(function(html) {
...
function show_quickmenu(type, subject, srctype, e)
{
    ...
    if (e.pageX || e.pageY) {
        ...
        posx = e.clientX + $('body').scrollLeft() + $('html').scrollLeft();
        posy = e.clientY + $('body').scrollTop() + $('html').scrollTop();
    }
    ...
    if (newdivleft + divwidth > $(window).innerWidth() - 10) {
        newdivleft = $(window).innerWidth() - divwidth - 10;
    }
    // Make sure we don't exceed edges:
    if (newdivleft < 0) {
        newdivleft = 0;
    }
    if (newdivtop + divheight > $(window).innerHeight() - 10) {
        newdivtop = $(window).innerHeight() - divheight -10 ;
    }
...
function show_quick_display(options)
{
    ...
    var per_page = $('#perpage').val();
    if (options !== undefined) {
        if (options.add_rows !== undefined && options.add_rows == 1) {
            add_rows = 1;
            ...
            if (x.error == 0) {
                if (add_rows == 0) {
                    ...
                    if ((used_height > max_height && height > max_height) || height < used_height) {
                        ...
                        var title_height = $('#popup_title').outerHeight();
                ...
                if (add_rows == 0) {
                    ...
                    $('#spotdetails_table>tbody tr').eq(-1).after(x.contents); // for old spots view
                }
            } else {
                update_message_bar(x.error);
            }
...
function fold_transfer(id, type)
{
    ...
    $.post(url, {
        ...
        if (x.error == 0) {
            ...
            if (type == 'post') {
                $('#data_post_' + id).toggle();
            } else {
                $('#data_down_' + id).toggle();
            }
        } else {
            update_message_bar(x.error);
        }
...
{
    $('#' + button_id).toggleClass('dynimgplus dynimgminus');
    $('#' + id).toggle();
    update_search_bar_height();
}
...
function collapse_select(name, par)
{
    ...
    $('#' + name + '_collapse').toggleClass('dynimgminus dynimgplus');
    if (par == 'size') {
        sel.attr('size', size);
    } else if (par == 'rows') {
        sel.attr('rows', size);
...
function submit_upload()
{
    ...
    $('#' + iframe_id).on('load',function() {
        var msg = $('#' + iframe_id).contents().find("body").html();
        $('#' + iframe_id).remove();
        update_message_bar(msg);
    });
...
function show_help(msg, header, th, xpos, ypos)
{
    ...
    $('#helpwrapper').css({ 'top': new_pos, 'left': left});
...
function update_widths(the_id)
{
    ...
    $('.donotoverflowdamnit').each(function() { $(this).width(10); });
    var oritextwidth = Math.round($('#' + the_id).outerWidth(true));
    ...
    var newtextwidth = $('#' + the_id).outerWidth(false);
...
{
    var _width = Math.round($(window).width() * 0.95);
    var _left = _width;
    var _right = 0, cnt = 0, last_width = 0, first_left = -1;
...
{
    var side_bar_width = emRemToPx('22em', 'body', false);
    ...
    if (sidebar || display === false) {
        ...
        $('#contentleft').outerWidth(side_bar_width + 1);
