...
class spot_viewer
{
    ...
    private function get_basic_browse_query($do_count = FALSE)
    {
        ...
        if (!$do_count) { 
            ...
            ' LEFT JOIN spot_images ON (spots."spotid" = spot_images."spotid") '; 
    ...
    {
        $sql = '"title", spots."size", spots."spotid", spots."stamp", spots."reports", spots."comments", spots."poster", spots."description", ' .
            '"category", "subcata", "subcatb", "subcatc", "subcatd", "subcatz", spots."url", "image", "image_file", "reference", ' . 
    ...
    public function get_spot_data($perpage, $offset, &$last_item)
    {
        ...
        // Get the set data
    //echo_debug_var_file('/tmp/foo', $sql2);
        $allsets = array();
