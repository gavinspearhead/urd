...
class urdd_rss
{
    ...
    public function update_rss_set($setid, $title, $timestamp, $description, $summary)
    {
        ...
        $this->db->update_query_2('rss_sets',
    ...
    public function update_feedcount($rss_id)
    {
        ...
        $this->db->execute_query($sql, array(':rss_id1'=>$rss_id, ':rss_id2'=>$rss_id));
    }

    public function rss_update(array $rss_info)
    {
        ...
        foreach ($rss->items as $item) {
            ...
            if ($timestamp >= $expire_time) {
                $id = $this->check_link($rss_id, $link);
                if ($id === FALSE) {// if it doesn't exist, add it
                    $this->insert_rss_set($rss_id, $link, $title, $timestamp, $description, $summary);
                    $newcnt++;
                } else {
                    $this->update_rss_set($id, $title, $timestamp, $description, $summary);
                    $updcnt++;
                }
            }
        }
        echo_debug("Updated $updcnt sets; added $newcnt sets", DEBUG_SERVER);
        $this->update_feedcount($rss_id);
    ...
    public function purge_rss($rss_id, $dbid=NULL)
    {
        ...
        $res = $this->db->select_query($qry, array(':rss_id' => $rss_id));
        if (!isset($res[0]['cnt'])) {
        ...
        $this->db->delete_query('usersetinfo', $sql, array($rss_id, $type));
        ...
        $sql = '"setID" IN (SELECT "setid" FROM rss_sets WHERE "rss_id"=?) AND "type"=?';
        $this->db->delete_query('extsetdata', $sql, array($rss_id, $type));
        ...
        $qry = '"rss_id"=?';
        $this->db->delete_query('rss_sets', $qry, array($rss_id));

        if ($dbid !== NULL) {
            update_queue_status($this->db, $dbid, NULL, 0, 90);
        }
        $this->update_feedcount($rss_id);
        $this->db->update_query_2('rss_urls', array('extset_update'=>0), '"id"=?', array($rss_id));
        fetch_rss::delete_cache_entry($rss_info['url'], get_magpie_cache_dir($this->db));
        if ($dbid !== NULL) {
    ...
    public function expire_rss($rss_id, $dbid)
    {
        echo_debug_function(DEBUG_MAIN, __FUNCTION__);
        assert(is_numeric($rss_id));
        $rss_info = get_rss_info($this->db, $rss_id);
        ...
        $res = $this->db->select_query($qry, $input_arr);
        if (isset($res[0]['cnt'])) {
            $cnt = $res[0]['cnt'];
        }
        ...
        $res = $this->db->delete_query('rss_sets', $qry, $input_arr);
        update_queue_status($this->db, $dbid, NULL, 0, 30);
        ...
        $res = $this->db->delete_query('usersetinfo', $sql, array($type));

        update_queue_status($this->db, $dbid, NULL, 0, 60);
        // ditto extsetinfo
        $sql = '"setID" NOT IN (SELECT "setid" FROM rss_sets) AND "type"=?';
        $res = $this->db->delete_query('extsetdata', $sql, array($type));
